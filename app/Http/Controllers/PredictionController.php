...
class PredictionController extends Controller
{
    ...
    {
        Log::info("=== PREDICTIVE COST ANALYSIS START ===");
        ...
        try {
            ...
            // Base cost calculation from parts analysis
            $costAnalysis = $this->calculateBaseCostFromParts($costAnalysis, $partsAnalysis);
            
            // Safety premium calculation
            $costAnalysis = $this->applySafetyPremium($costAnalysis, $safetyAnalysis);
            
            // Vehicle characteristics cost adjustment
            $costAnalysis = $this->applyVehicleCharacteristicsCost($costAnalysis, $vehicleHistory, $currentMileage);
            
            // Urgency premium
            $costAnalysis = $this->applyUrgencyPremium($costAnalysis, $partsAnalysis, $safetyAnalysis);
            
            // Final cost calculation
            $costAnalysis = $this->calculateFinalCost($costAnalysis);
            
            Log::info("✅ Cost analysis completed - Range: RM " . number_format($costAnalysis['total_estimated_cost']['min']) . " - RM " . number_format($costAnalysis['total_estimated_cost']['max']));
            
            return $costAnalysis;
            
        } catch (\Exception $e) {
            Log::error("Cost analysis error: " . $e->getMessage());
            return $this->getDefaultCostAnalysis();
        }
    }
    ...
    {
        try {
            // Ensure cost_estimate has the correct structure
            $costEstimate = [
                'min' => $costAnalysis['total_estimated_cost']['min'] ?? 200,
                'max' => $costAnalysis['total_estimated_cost']['max'] ?? 500
            ];
            
            $recommendations = [
                'priority' => 'routine',
                'action_plan' => [],
                'cost_estimate' => $costEstimate,  // <- Fixed: ensure this structure
            ...
            if ($safetyAnalysis['breakdown_risk'] === 'critical') {
                $recommendations['priority'] = 'critical_safety';
                $recommendations['safety_priority'] = true;
                $recommendations['action_plan'][] = '🚨 CRITICAL SAFETY: Immediate vehicle inspection required';
                $recommendations['time_estimate'] = 'Same day';
                
                foreach ($safetyAnalysis['critical_alerts'] as $alert) {
                    $recommendations['action_plan'][] = "🛑 SAFETY: " . $alert['message'];
                }
            } elseif ($safetyAnalysis['breakdown_risk'] === 'high') {
                $recommendations['priority'] = 'high_safety';
                $recommendations['safety_priority'] = true;
                $recommendations['action_plan'][] = '⚠️ HIGH SAFETY RISK: Urgent inspection within 48 hours';
            }

            // PARTS ANALYSIS WITH COST INSIGHTS
            if (!empty($partsAnalysis['immediate'])) {
                if (!$recommendations['safety_priority']) {
                    $recommendations['priority'] = 'immediate';
                }
                $recommendations['action_plan'][] = '🔧 IMMEDIATE: ' . count($partsAnalysis['immediate']) . ' parts need attention';
            } elseif (!empty($partsAnalysis['soon'])) {
                if ($recommendations['priority'] === 'routine') {
                    $recommendations['priority'] = 'high';
                }
                $recommendations['action_plan'][] = '⚡ SOON: ' . count($partsAnalysis['soon']) . ' items need service';
            } else {
                if ($recommendations['priority'] === 'routine') {
            ...
            if ($costAnalysis['cost_confidence'] === 'high') {
                $recommendations['action_plan'][] = '💰 COST: High confidence estimate based on detailed analysis';
            }
            
            if ($costAnalysis['safety_premium'] > 0) {
                $recommendations['action_plan'][] = '🛡️ COST: Safety premium applied due to critical issues';
            }

            // DEBUG: Log the final recommendations structure
            Log::info("Enhanced recommendations created:", [
                'priority' => $recommendations['priority'],
                'cost_estimate' => $recommendations['cost_estimate'],
                'cost_confidence' => $recommendations['cost_confidence']
            ]);
            ...
            // Return safe fallback with guaranteed cost structure
            return [
                'priority' => 'routine',
                'action_plan' => ['📅 Schedule regular maintenance (enhanced analysis unavailable)'],
                'cost_estimate' => ['min' => 200, 'max' => 500],  // <- Guaranteed structure
                'time_estimate' => '2-4 hours',
                'safety_priority' => false,
                'cost_confidence' => 'low',
...
private function getVehicleHistory($vehicleNumber)
{
    try {
        ...
        $daysSinceLast = $lastService ? Carbon::parse($lastService->Datereceived)->diffInDays(now()) : 365;
...
private function processServiceHistory($records)
{
    return $records->map(function ($record) {
        return [
            ...
            'days_ago' => $record->Datereceived ? Carbon::parse($record->Datereceived)->diffInDays(now()) : 'Unknown'
        ];
    });
}
...
private function calculateServicePatterns($records)
{
    ...
    if ($oldestService && $newestService && $oldestService->Datereceived && $newestService->Datereceived) {
        try {
            $monthsSpan = max(1, Carbon::parse($oldestService->Datereceived)->diffInMonths(Carbon::parse($newestService->Datereceived)));
        } catch (\Exception $e) {
            $monthsSpan = 1;
        }
    }
...
private function safePartsAnalysis($currentMileage, $vehicleHistory, $mlPrediction)
    {
        try {
            ...
            $fleetParts = [
                // PRIORITY 1 - Critical Safety Parts (from Priority 1.txt analysis)
                'Engine Oil & Hydraulics' => [
                    'priority' => 1,
                    'interval_km' => 10000,  // Updated: Commercial vehicles need oil change every 10K KM
                    'cost_range' => ['min' => 200, 'max' => 400],  // Updated: Commercial oil + hydraulic fluid
                    'keywords' => [
                        ...
                        'minyak auto', 'minyak stereng', 'power steering leaking', 'oil sensor'
                    ],
                    'is_critical' => true,
                    'industry_note' => 'Critical for engine protection - commercial grade oil required'
                ],
                'Brake System' => [
                    'priority' => 1,
                    'interval_km' => 50000,  // Updated: Commercial brake pads last 40K-60K KM
                    'cost_range' => ['min' => 600, 'max' => 1500],  // Updated: Commercial brake system is expensive
                    'keywords' => [
                        'brake', 'brek', 'adjust brake', 'brake chamber', 'lining brake',
                        'brake jammed', 'brake berbunyi', 'angin bocor', 'pad brek',
                        'brake issue', 'brake fail', 'pedal kosong', 'handbrake'
                    ],
                    'is_critical' => true,
                    'industry_note' => 'Safety critical - includes pads, chambers, air system'
                ],
                'Oil Seals' => [
                    'priority' => 1,
                    'interval_km' => 100000,  // Updated: Oil seals last longer, 80K-120K KM
                    'cost_range' => ['min' => 300, 'max' => 800],  // Updated: Labor intensive replacement
                    'keywords' => [
                        'oil seal', 'oilseal', 'seal minyak', 'sealing', 'shaft seal',
                        'oil seal axle', 'oil seal rosak', 'bocor'
                    ],
                    'is_critical' => true,
                    'industry_note' => 'Prevents oil leaks - labor intensive replacement'
                ],
                'Gearbox/Transmission' => [
                    'priority' => 1,
                    'interval_km' => 120000,  // Updated: Major service at 100K-150K KM
                    'cost_range' => ['min' => 2000, 'max' => 8000],  // Updated: Very expensive for commercial vehicles
                    'keywords' => [
                        'gearbox', 'gear oil', 'transmission', 'kotak gear', 'minyak gearbox',
                        'gear tak boleh masuk', 'gearbox bocor', 'gear lever', 'gear susah'
                    ],
                    'is_critical' => true,
                    'industry_note' => 'Major component - rebuild/replacement very expensive'
                ],

                // PRIORITY 2 - Important Maintenance Parts (from Priority 2.txt analysis)
                'Tires & Wheels' => [
                    'priority' => 2,
                    'interval_km' => 100000,  // Updated: Commercial tires last 80K-120K KM depending on roads
                    'cost_range' => ['min' => 800, 'max' => 2000],  // Updated: Commercial tires expensive, multiple per vehicle
                    'keywords' => [
                        'tayar', 'tire', 'tukar tayar', 'tayar pancit', 'tayar botak',
                        'tayar meletup', 'rim', 'alignment', 'rotate tayar', 'tampal tayar',
                        'tayar bocor', 'tayar celup', 'alih pasang', 'wheel'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Commercial grade tires - 6-10 tires per vehicle'
                ],
                'Suspension & Absorber' => [
                    'priority' => 2,
                    'interval_km' => 120000,  // Updated: Heavy duty suspension lasts 100K-150K KM
                    'cost_range' => ['min' => 800, 'max' => 2000],  // Updated: Heavy duty commercial parts
                    'keywords' => [
                        'absorber', 'spring', 'bush', 'suspension', 'shock absorber',
                        'absorber leaking', 'spring patah', 'bush trailer', 'torque rod',
                        'equalizer bush', 'absorber kabin', 'getah'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Heavy duty suspension for loaded vehicles'
                ],
                'Electrical & Lighting' => [
                    'priority' => 2,
                    'interval_km' => 50000,  // Updated: Bulbs and wiring wear from vibration
                    'cost_range' => ['min' => 150, 'max' => 500],  // Updated: Multiple lights, wiring harnesses
                    'keywords' => [
                        'lampu', 'wiring', 'electrical', 'signal', 'lighting', 'fius',
                        'battery', 'bateri', 'alternator', 'lampu depan', 'lampu brake',
                        'lampu trailer', 'lampu belakang', 'horn', 'mentol'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Safety lighting - LED upgrades available'
                ],
                'Air System' => [
                    'priority' => 2,
                    'interval_km' => 40000,  // Updated: Air brake system critical for heavy vehicles
                    'cost_range' => ['min' => 500, 'max' => 1200],  // Updated: Air compressor, valves, chambers
                    'keywords' => [
                        'angin bocor', 'air bocor', 'belon bocor', 'angin tidak naik', 
                        'chamber bocor', 'air system', 'compressor', 'air dryer'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Critical for air brake operation'
                ],

                // PRIORITY 3 - Routine Maintenance Parts (from Priority 3.txt analysis)
                'Cooling System' => [
                    'priority' => 3,
                    'interval_km' => 60000,  // Updated: Radiator service, coolant change every 60K KM
                    'cost_range' => ['min' => 400, 'max' => 1000],  // Updated: Commercial radiators larger, coolant expensive
                    'keywords' => [
                        'coolant', 'air coolant', 'radiator', 'cooling system',
                        'air panas', 'coolant bocor', 'radiator bocor', 'belt'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Prevents overheating - critical in tropical climate'
                ],
                'Fuel System' => [
                    'priority' => 3,
                    'interval_km' => 80000,  // Updated: Fuel filters, injector cleaning
                    'cost_range' => ['min' => 300, 'max' => 800],  // Updated: Diesel system maintenance
                    'keywords' => [
                        'tangki minyak', 'fuel', 'injector', 'tangki bocor',
                        'penutup tangki', 'fuel system', 'tangki sawit', 'fuel filter'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Diesel system - fuel quality affects maintenance interval'
                ],
                'Clutch System' => [
                    'priority' => 2,
                    'interval_km' => 150000,  // Added: Based on fleet data showing clutch issues
                    'cost_range' => ['min' => 1500, 'max' => 3500],  // Added: Heavy duty clutch expensive
                    'keywords' => [
                        'clutch', 'kopling', 'clutch rosak', 'clutch slip',
                        'clutch plate', 'clutch pump', 'gear susah masuk'
                    ],
                    'is_critical' => false,
                    'industry_note' => 'Heavy duty clutch for loaded operations'
                ]
            ];
            ...
            foreach ($fleetParts as $partName => $partData) {
                ...
                $daysRemaining = $dailyUsageKm > 0 ? max(0, round($kmRemaining / $dailyUsageKm)) : 0;
...
private function findLastServiceForFleetPart($records, $keywords)
    ...
    private function calculateDaysAgo($date)
    {
        try {
            ...
            return Carbon::parse($date)->diffInDays(now()) . ' days ago';
        } catch (\Exception $e) {
            return 'Unknown';
        }
    }
